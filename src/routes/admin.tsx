...
import {
  ...
  Printer, Lock, Filter, TrendingUp,
} from "lucide-react";
import {
...
import {
  ...
  downloadAuditLog, downloadInternsReport, downloadStaffReport,
  type StaffRecord,
} from "@/lib/admin-pdf";
...
function JobsTab({ jobs, isExpired, addJob, updateJob, deleteJob, onViewApps }: any) {
  const [editing, setEditing] = useState<null | (Omit<Job, "id" | "abbr"> & { id?: number })>(null);
  const open = (j?: Job) => setEditing(j ? { ...j } : { ...emptyJob });
  const save = () => {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => open()} className="px-3 py-1.5 text-sm bg-caa-navy text-white rounded-md hover:bg-caa-navy-2 inline-flex items-center gap-1"><Plus className="h-4 w-4" /> New listing</button>
      </div>
      <div className="caa-card overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-caa-surface text-xs text-caa-muted">
            <tr><th className="text-left p-3">Title</th><th className="text-left p-3">Visibility</th><th className="text-left p-3">Band</th><th className="text-left p-3">Closes</th><th className="text-left p-3">Status</th><th className="text-right p-3">Actions</th></tr>
          </thead>
          <tbody className="divide-y divide-caa-border">
            {jobs.map((j: Job) => (
              <tr key={j.id}>
                <td className="p-3"><p className="font-medium text-caa-body">{j.title}</p><p className="text-[11px] text-caa-muted">{j.dept}</p></td>
                <td className="p-3"><span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${j.visibility === "internal" ? "bg-caa-navy-2/15 text-caa-navy-2" : "bg-caa-success/10 text-caa-success"}`}>{j.visibility}</span></td>
                <td className="p-3 text-xs">{j.salaryBand}</td>
                <td className="p-3 text-xs">{j.closes}</td>
                <td className="p-3">{isExpired(j) ? <span className="px-2 py-0.5 rounded-full bg-caa-danger/10 text-caa-danger text-[10px]">Expired</span> : <span className="px-2 py-0.5 rounded-full bg-caa-success/10 text-caa-success text-[10px]">Active</span>}</td>
      ...
      {editing && (
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="caa-card p-5 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <h3 className="font-bold text-lg mb-4">{editing.id ? "Edit listing" : "New job listing"}</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Field label="Title"><input className={fi} value={editing.title} onChange={(e) => setEditing({ ...editing, title: e.target.value })} /></Field>
              ...
              <Field label="Salary range"><input className={fi} value={editing.salary} onChange={(e) => setEditing({ ...editing, salary: e.target.value })} /></Field>
              ...
              <Field label="Visibility"><select className={fi} value={editing.visibility} onChange={(e) => setEditing({ ...editing, visibility: e.target.value as Visibility })}><option value="external">External</option><option value="internal">Internal (staff only)</option></select></Field>
              ...
              <div className="sm:col-span-2"><Field label="Description"><textarea rows={3} className={fi} value={editing.description ?? ""} onChange={(e) => setEditing({ ...editing, description: e.target.value })} /></Field></div>
            </div>
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setEditing(null)} className="px-3 py-1.5 text-sm border border-caa-border rounded-md">Cancel</button>
              <button onClick={save} className="px-3 py-1.5 text-sm bg-caa-navy text-white rounded-md">{editing.id ? "Save changes" : "Create listing"}</button>
            </div>
          </div>
        </div>
      )}
